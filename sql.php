	...
	return "
		select
		  Company.CompanyID as legacy_company_id,
			...
			where [Order].intOrderID=Company.OrderID order by dteDate desc) as referral_code,
			IntroTitle as profile_title,
			CONVERT(TEXT,MarketingMsg) as profile_description,
			(select top 1 Path from LogosAndImages where OwnerID=Profiles.intProfileID and LogosAndImages.ImageType=104) as profile_photo_url
			from Profiles
			join Company on Company.CompanyID=Profiles.intOwnerID
			where (select top 1 ReferralCode.referralCode from [Order]
			left outer join ReferralCode on [Order].intReferralCodeID=ReferralCode.intReferralCodeID
			where [Order].intOrderID=Company.OrderID order by dteDate desc) is not null and
			(select top 1 ReferralCode.referralCode from [Order]
			left outer join ReferralCode on [Order].intReferralCodeID=ReferralCode.intReferralCodeID
			where [Order].intOrderID=Company.OrderID order by dteDate desc) <> 'FREELISTINGS'
