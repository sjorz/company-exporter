...
class Table
{
    ...
    //**********************************************************************
    //
    //  As rows. An alternative for asArray, as that sometimes generates a
    //  segmentation fault. See comment there.
    //
    //**********************************************************************
	...
	$rows = array();
	    ...
	    for ($i = 1; $i <= odbc_num_fields($this->resultSet); $i++)
            {
		$column = odbc_field_name ($this->resultSet, $i);
                $val = odbc_result ($this->resultSet, $i);
		$cols [$column] = $val;
                //printf ("%s=>%s\n", $column, $val);
            }
	    $rows[] = $cols;
    ...
    //
    // 	NOTE: Function odbc_fetch_array will generate a segmentation fault
    //  if the result set contains a row having one or more empty column
    //  names. That typically happens if a sub query is used and the 'as'
    //  clause (to name the resulting field) is omitted!!!
    //
    //**********************************************************************
